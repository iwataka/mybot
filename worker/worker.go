...
const (
	// StatusActive means worker is active, able to respond to WorkerSignal
	// but doesn't indicate whether started or stopped.
	StatusActive = iota
...
func activateWorker(inChan chan *WorkerSignal, outChan chan interface{}, worker Worker, timeout time.Duration) {
	...
	for workerSignal := range inChan {
		...
		if signal == RestartSignal || signal == StopSignal || signal == KillSignal {
			...
			if status && (force || timestamp.Before(t)) {
				...
				case <-time.After(timeout):
					sendNonBlockingly(outChan, WorkerStatus(StatusInactive), timeout)
				}
				status = false
			}
		...
		if signal == PingSignal {
			if outChan != nil {
				outChan <- WorkerStatus(StatusActive)
			}
		}
	}
