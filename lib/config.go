...
type APIConfig struct {
	...
	TimestampTemplate string `json:"timestamp_template" toml:"timestamp_template"`
}
...
func (c *APIConfig) Message(checkTimestamp bool) (string, error) {
	...
	// Check timestamp and return an empty string if data is old.
	if checkTimestamp && len(c.TimestampTemplate) != 0 {
		tmpl, err := template.New("message").Parse(c.TimestampTemplate)
		if err != nil {
			return "", err
		}
		buf := new(bytes.Buffer)
